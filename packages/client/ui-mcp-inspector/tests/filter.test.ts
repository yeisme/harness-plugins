...
const items: readonly ToolHubItemV1[] = [
  { id: 'skill:writer', family: 'skill', origin: 'skill', name: 'writer', label: 'writer', description: 'Write docs', source: 'user', availability: 'available', enabled: true, canToggle: true, purpose: { zh: '审阅和撰写文档', category: 'writing', searchTerms: ['审阅', 'review'] } },
...
describe('filterCatalog', () => {
  it('filters by query, family, and enabled state', () => {
    ...
    expect(filterCatalog(items, { query: '审阅', family: 'all', enabled: 'all' }).map(item => item.id)).toEqual(['skill:writer'])
    expect(filterCatalog(items, { query: '', family: 'all', enabled: 'all', purpose: 'writing' }).map(item => item.id)).toEqual(['skill:writer'])
  })

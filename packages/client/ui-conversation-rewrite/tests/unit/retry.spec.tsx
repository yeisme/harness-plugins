...
describe('RetryButton', () => {
  it('renders an enabled retry button and fires the action', () => {
    ...
    expect(button.getAttribute('title')).toBe('重试')
    expect(button.querySelector('svg')?.getAttribute('data-test-icon')).toBe('refresh')
  ...
  it('renders loading and disables clicks', () => {
    ...
    expect(button.querySelector('svg')?.getAttribute('data-test-icon')).toBe('loading')
    expect(button.textContent).toBe('')
    fireEvent.click(button)

...
describe('Subagent Monitor apply', () => {
  it('explains why Agents is unavailable before a session is selected', () => {
    ...
    render(host.registered.find(entry => entry.name === 'sidebar.footer.action')!.component?.() as never)
  ...
  it('registers one icon-backed Agents sidebar launcher and opens the pane', () => {
    ...
    expect(registered.filter(entry => entry.name === 'conversation.view')).toHaveLength(1)
    render(registered.find(entry => entry.name === 'sidebar.footer.action')!.component?.({ wide: true }) as never)
    const button = screen.getByRole('button', { name: 'Subagents' })
  ...
  it('hot-plugs paneWorkbench after activation and enables the launcher', () => {
    ...
    expect(host.registered.filter(entry => entry.name === 'sidebar.footer.action')).toHaveLength(1)
    render(host.registered.find(entry => entry.name === 'sidebar.footer.action')!.component?.({ wide: true }) as never)
    const button = screen.getByRole('button', { name: 'Subagents' })

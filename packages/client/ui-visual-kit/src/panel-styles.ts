...
export function buildPanelStyles(options: BuildPanelStylesOptions): string {
  ...
  const accent = options.accentFallback === undefined
    ? panelVar('accent')
    : `var(--dsw-alias-accent,var(--dsw-alias-state-business-primary,${options.accentFallback}))`
  const s = PANEL_SCALE
...
${root} .vk-btn[data-primary='true']{color:var(--vk-text-primary);background:color-mix(in srgb,var(--vk-accent) 28%,transparent);border-color:color-mix(in srgb,var(--vk-accent) 62%,transparent)}
...
${root} .vk-badge{display:inline-flex;align-items:center;min-height:20px;padding:0 7px;border-radius:999px;background:color-mix(in srgb,var(--vk-text-primary) 7%,transparent);color:var(--vk-text-secondary);font-size:${s.font.micro}}
...
${root} .vk-progress{height:4px;overflow:hidden;border-radius:999px;background:var(--vk-fill-hover)}
${root} .vk-progress>i{display:block;height:100%;background:var(--vk-accent);border-radius:inherit}
${root} .vk-empty{display:grid;place-items:center;align-content:center;gap:var(--vk-gap-sm);min-width:0;min-height:92px;padding:16px;text-align:center;color:var(--vk-text-tertiary);background:color-mix(in srgb,var(--vk-bg-layer-1) 72%,transparent);border:1px solid var(--vk-border-l1);border-radius:var(--vk-radius-md)}
${root} .vk-empty>strong{margin:0;color:var(--vk-text-primary);font-size:var(--vk-font-strong);font-weight:650}
${root} .vk-empty>p,${root} .vk-empty>small{max-width:64ch;margin:0;color:var(--vk-text-tertiary);font-size:var(--vk-font-small);line-height:1.45}
...
${root} .vk-skeleton::after{position:absolute;inset:0;background:linear-gradient(90deg,transparent,color-mix(in srgb,var(--vk-text-primary) 6%,transparent),transparent);transform:translateX(-100%);animation:vk-shimmer-${scope} 1.4s ease-in-out infinite;content:''}

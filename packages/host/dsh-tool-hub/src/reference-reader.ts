...
export class SkillReferenceReader {
  ...
  async readSkill(input: SkillReferenceReadInputV1, signal: AbortSignal): Promise<SkillReferenceReadAnswerV1> {
    ...
    try {
      ...
      signal.throwIfAborted()
      const matches = catalog.filter(item => { const rec = record(item); return rec?.name === name && rec?.source === input.source })
      const summary = matches.length === 1 ? fileIdentity(matches[0]) : undefined
      if (!summary) return failure('denied', 'source_not_readable')
      ...
      const current = (await waitForOwner(owner.list({ signal }), signal)).filter(item => { const rec = record(item); return rec?.name === name && rec?.source === input.source })
      signal.throwIfAborted()

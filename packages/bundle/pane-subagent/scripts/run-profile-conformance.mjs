...
try {
  ...
  const faceProbe = run('node', [
    '--input-type=module',
    '-e',
    "const fs = await import('node:fs'); const { createRequire } = await import('node:module'); const code = fs.readFileSync('./lib/client.js', 'utf8'); if (!code.startsWith('window.__ModuleLoader__.load({')) throw new Error('client bundle is not a __ModuleLoader__.load registration'); if (!code.includes('id: \"@yeisme/dsh-pane-subagent\"')) throw new Error('client bundle registers the wrong id'); if (!code.includes('factory: (require)')) throw new Error('client bundle is missing the factory face'); const nodeRequire = createRequire(import.meta.url); const primitiveStub = new Proxy({}, { get: () => () => null }); const platformRequire = id => id === '@deepseek-ai/dsh-client-ui-primitives' ? primitiveStub : nodeRequire(id); let face; globalThis.window = { __ModuleLoader__: { load: definition => { face = definition.factory(platformRequire) } } }; await import('./lib/client.js?probe=' + Date.now()); if (!face || !Array.isArray(face.inject)) throw new Error('client bundle did not export inject'); if (face.inject.includes('paneWorkbench')) throw new Error('optional paneWorkbench capability became a loader dependency'); console.log('client-bundle-ok')",
  ], bundleRoot)

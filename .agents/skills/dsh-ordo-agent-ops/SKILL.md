...
Use this skill for the DSH host plugin, Web client module, profile bundle, ToolView, typed client service, event subscription, safe action dispatch, and their tests and documentation. Use the local [Ordo Agent Ops OpenSpec](../../../openspec/changes/ordo-dsh-plugin-visualization-v1/README.md) as the DSH-facing design source. Ordo remains the owner of run, task, attempt, session, runtime, lease, worktree, approval, verification, evidence, and closeout facts.
...
For a code change, run the narrow DSH package tests and the repository documentation gates selected by the diff:

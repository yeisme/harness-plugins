# workbench-harness-route-wiring Specification

## Purpose

追溯性留痕：五个 Harness 组件挂接 /harness 路由槽位与 iframe bridge 时序机接线的既成实现。descriptor catalog 缺失或未批准时一律 fail-closed 呈现 needs_contract 占位，不渲染供给数据；bridge 消息按 per-load session 强制 handshake→ready→view.request 时序，乱序/漂移即卸载。本 capability 不引入新行为。

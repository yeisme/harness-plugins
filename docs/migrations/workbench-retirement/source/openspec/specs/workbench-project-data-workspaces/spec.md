# workbench-project-data-workspaces Specification

## Purpose

Project data next-slice 决策合同：首期切片（四视图、五角色、9 custom field
kinds、首发 trigger 与 Owner canary）交付后的下一批 capability 排期必须
基于真实使用/性能证据逐项做出 deliver-next、retain-next 或 reject-now
决定，retain-next 项不得被静默删除，reopen 时须重新做 fit/split-owner
评估。

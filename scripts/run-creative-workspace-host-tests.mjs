...
if (process.argv.slice(2).some(arg => !['--touch', '--zoom'].includes(arg))) throw new Error('Supported options: --touch, --zoom')
const touch = process.argv.includes('--touch')
const zoom = process.argv.includes('--zoom')
const command = `node scripts/run-creative-workspace-host-tests.mjs${touch ? ' --touch' : ''}${zoom ? ' --zoom' : ''}`
...
const exec = (cmd, args, cwd = source) => new Promise(resolveResult => {
  const child = spawn(cmd, args, { cwd, env: { ...process.env, pnpm_config_verify_deps_before_run: 'warn', DSH_CREATIVE_WORKSPACE_TOUCH: touch ? '1' : '0', DSH_CREATIVE_WORKSPACE_ZOOM: zoom ? '1' : '0', DSH_SNAPSHOT: 'replay', DSH_TELEMETRY_DISABLED: '1', DSH_CREATIVE_WORKSPACE_PLUGIN_ROOT: root, DSH_CREATIVE_WORKSPACE_EVIDENCE_DIR: resolve(dir, 'artifacts'), DSH_CREATIVE_CREATOR_HOST_MODULE: new URL('../packages/bundle/dsh-creator-studio/lib/index.js', import.meta.url).href }, stdio: ['ignore', 'pipe', 'pipe'] })
  let stdout = '', stderr = ''
...
const fingerprint = async () => {
  ...
  for (const [label, cwd, paths] of [
    ['host', source, ['apps/web/tests/creative-workspace.e2e.ts', 'apps/web/tests/creative-workspace-overlay.yml', 'apps/web/tests/creative-workspace-fixture', 'apps/web/tests/scaffold.ts', 'packages/skill/skill-badge/assets/dsh-badge.png', 'apps/web/tests/support.ts', 'packages/api/session-controller', 'packages/context/session-reference', 'packages/client/modules', 'packages/client/ui-conversation', 'vitest.web.config.ts', 'package.json', 'pnpm-lock.yaml']],
    ['plugins', root, ['packages/host/creator-studio', 'packages/client/ui-creator-studio', 'packages/client/ui-visual-kit', 'packages/client/ui-surface', 'packages/bundle/dsh-creator-studio', 'packages/host/dsh-browser-host', 'packages/client/ui-browser-pane', 'packages/bundle/dsh-browser-pane', 'packages/client/ui-pane-workbench', 'packages/bundle/dsh-desktop-workbench', 'packages/bundle/pane-workbench', 'packages/bundle/dsh-rich-media', 'packages/bundle/dsh-mermaid-render', 'packages/client/ui-mermaid-render', 'packages/client/ui-structured-content', 'scripts/run-creative-workspace-host-tests.mjs', 'package.json', 'pnpm-lock.yaml']],
  ]) {
    ...
    for (const path of [...new Set(listing.stdout.split('\0').filter(Boolean))].sort()) {
      ...
      // Package documentation is not loaded by this fixed synthetic scenario.
      // Keep code, fixtures, manifests and configuration in the source fence.
      if (/(^|\/)README(?:\.[a-z-]+)?\.md$/iu.test(path)) continue
      let digest
...
await Promise.all([
  ...
  writeFile(resolve(dir, 'summary.json'), JSON.stringify({ schema_version: 'yeisme.integration_test_evidence.v1', project: 'agent/harness-plugins', run_id: runId, layer: 'e2e', command, touch_emulation: touch, native_zoom: zoom, status: exitCode ? 'failed' : 'passed', exit_code: exitCode, test_exit_code: result.code, source_inputs_unchanged: inputsUnchanged, started_at: new Date(started).toISOString(), finished_at: new Date().toISOString(), duration_ms: Date.now() - started, evidence: { stdout: 'stdout.log', stderr: 'stderr.log', command: 'command.txt', env: 'env.json', artifacts: 'artifacts/' }, redaction: { enabled: true, policy: 'synthetic keyless scenario; credentials and local paths redacted' }, source_diff_stat_unchanged: before.stdout === after.stdout, tests: ['creative-workspace.e2e.ts'], scope: 'real Host and ModuleLoader with synthetic Creator owner; Browser provider absent and entry withheld', real_domain_acceptance: false, real_viewport_provider: false }, null, 2)),
])

...
const commands = [
 ['pnpm',['--filter','@yeisme/dsh-creator-studio-host','run','typecheck']],
 ['pnpm',['--filter','@yeisme/dsh-creator-studio-host','exec','vitest','run','tests/input-intake.spec.ts']],
]
...
writeFileSync(join(artifactsDir, 'contract.txt'), 'MCP input -> host file selection -> bounded HTTP -> safe owner reference\n')

process.stdout.write(`creative input integration evidence: ${rel(evidenceDir)}\n`)

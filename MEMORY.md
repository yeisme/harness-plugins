## 2026-09-12: dsh-provider-presets-v1 — cc-switch 式渠道市场落地（本会话）

- OpenSpec change `dsh-provider-presets-v1` 全任务勾选（strict valid + verification.md）：新两包 `packages/client/ui-provider-presets` + `packages/bundle/dsh-provider-presets`（无 host 半，web 行空 apply）。在 Models 设置页预留的 `settings.models.footer` 扩展 slot 渲染「渠道市场」：9 条预设渠道（GLM anthropic/openai、Kimi、硅基、OpenRouter、百炼、ModelScope、火山方舟、DeepSeek 官方 + 自定义卡）→ 引导添加（`remote.llm.discoverModels` GET /models 拉取含耗时 → 勾选 → `credentials.set` 入 host refs + `settings.mutate('llm-pi-ai')` set providers.<route>，容量缺省字段省略走 route 默认）→ 总览「设为默认」整段写 `agent-default-model`（live）。建议 route key 全部避开 pi-ai 内置 catalog 34 名（deepseek/openrouter/moonshotai/zai…）。
- 版本策略：仓内 pin rc.6 类型面，footer slot + `remote.settings/credentials/llm` 子命名空间只在 0.1.2-rc.1（用户运行时）→ 本地结构化 wire 类型镜像（wire.ts），静态 inject 仅 `['locale']`。
- 验证：两包 50 tests + check:surfaces/bundles(29/29)/plugins（safe-projection 经 owner 复核豁免登记 presets.ts RAW_URL_LITERAL）+ 集成运行器 14/14 passed（temp/integration-test-runs/2026-09-12T14-46-*/：echo provider 拉取 2 models/27ms → 保存落盘 settings.yaml/.credentials.yaml → 设默认 → 卸载保留 user 层）；9 预设端点匿名探测全在线（7×401 鉴权门 + 3×200 公开列表）。
- 坑（详见 auto-memory dsh-browser-guard-facade / dsh-plugin-pack-file-chain）：①浏览器 guard facade 拦未声明 inject 的服务属性访问（连服务值上的嵌套属性都拦），必须 `ctx.get('remote.settings')` 点分名 + `ctx.on('internal/service')` 晚绑定；`$on` 从服务对象解构后必须 `.call(root)` 绑定。②未发布 @yeisme workspace 包安装进一次性 profile 要四层 file: tarball 重写链（visual-kit→surface→client→bundle），同版本重装先 remove。③pnpm pack 打的是 lib/，改码忘 build 会打进旧产物。④集成运行器开 web 必须用 boot 输出的带 token 完整 URL。
- 并行 lane 红（非本 lane）：ordo-agent-ops dispose-hmr 1 finding（未跟踪 project-owner.ts）、ui-ordo-agent-ops tsdown css?inline 解析失败（阻塞 `pnpm -r run build`）、tool-hub 2 环境门测试（需专用 env）、drama/rich-media/pane-workbench 测试红——全部位于 09-11/09-12 在途 WIP 文件。
...
## 2026-09-13
- radar-market-6-18-wave: dsh-radar-market-intelligence-v1 0→6/18（1.2 probe 门禁/1.4 typed mutation+对账/2.4 已读关注动作/3.1 问答草稿/3.2 隔离/3.3 bundle+docs），提交 8f61045；host 132+client 37 测试、integration:evidence market_mutation_reconcile 段、check:bundles/plugins/surfaces 全绿。坑：①bundle smoke react 解析=基线预存红（HEAD worktree 复现），修 moduleRequire 按 client 包树解析+ui-primitives stub；②test:visual 环境阻断（chromium 缺 libnspr4 等系统库，apt 无网），证据 ui-visual-2026-09-13T19-18-*；③exactOptionalPropertyTypes 下可选 prop 传 undefined 须用条件展开；④prompt-reference 大 change 剩余 35 任务全为真实领域/开发环境/音视频 seam 外部门（其 completion-audit 自证），未动。

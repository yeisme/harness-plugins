...
const checkout = resolve(process.argv[2] ?? 'temp/dsh-tools-pane-host')
...
const testFile = 'packages/client/ui-settings-plugins/tests/apply.client.spec.ts'
const command = `pnpm exec vitest run ${testFile}`

function redact(input) {
  return input.replaceAll(checkout, '<dsh-checkout>').replaceAll(projectRoot, '.')

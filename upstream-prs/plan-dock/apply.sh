#!/usr/bin/env bash
# Apply this archived fork change onto a clean dsh checkout at base 141eb6fef834.
# Usage: ./apply.sh /path/to/deepseek-harness-checkout
set -euo pipefail
repo="${1:?usage: apply.sh <deepseek-harness-checkout>}"
script_dir="$(cd "$(dirname "$0")" && pwd)"
git -C "$repo" apply --check "$script_dir/changes.patch"
git -C "$repo" apply "$script_dir/changes.patch"
if [ -d "$script_dir/new-files" ]; then
  (cd "$script_dir/new-files" && tar cf - .) | (cd "$repo" && tar xf -)
fi
echo "applied plan-dock"

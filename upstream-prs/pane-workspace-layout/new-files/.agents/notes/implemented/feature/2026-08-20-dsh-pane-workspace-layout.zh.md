...
AppFrame 求解三列两行布局。DSH 会话侧边栏跨越两行，会话占据主列上行，Bottom 工作区只占主列下行。Tool Details 只在 Right/Bottom Core Pane 内渲染。无论 dock、Sheet 还是最大化，工作区的左边界都不得越过实际侧边栏边缘。

扩展通过 `ctx.workspaceLayout.attach(ownerId, initialPreference, corePaneHost)` 连接。唯一 owner 句柄提供 `update()`、`getSnapshot()`、`subscribe()` 和 `dispose()`；必需 host 只接受封闭的 `dsh.tool-details` id。Right/Bottom owner props 暴露 `renderCoreView(id)`，使 AppFrame 继续拥有既有 Details occupant，而 DSH core 无需依赖扩展包。重复 owner 在 attach 时立即失败。owner dispose 后，两个 slot 投影、Core adapter、44px 活动轨道与所有工作区尺寸预留一并消失。
...
- AppFrame 不再包含独立 Details 几何；`ctx.layout.openDetails()` 要求已 attach 的 Core host，并把 Tool Details 路由进共享 Pane。
...
旧版 DSH 如果缺少两个 workspace slot、`ctx.workspaceLayout` 或 `workspace.core-pane.v1`，Core Pane bundle 会给出明确兼容错误。生产代码不回退到 `shell.overlay`、固定 `280px` 偏移、sidebar DOM 探测、第二套 Pane store 或独立 Details 列。

## 验证

- geometry 测试覆盖 1440、1243、1024、768 和 390px 宽度，包含 dock、Sheet、无独立 Details 几何和最大化。
- AppFrame 与 service 覆盖封闭 id 路由、缺少必需 host 时失败、单一 Core renderer、Session 切换关闭、指针与键盘 resize、dispose 和侧边栏边界不变式。
...
- `ctx.layout.openDetails()` 保持公开 API 不变，但要求并只路由到 Core Pane owner。

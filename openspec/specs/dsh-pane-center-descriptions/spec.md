# dsh-pane-center-descriptions Specification

## Purpose
定义 DSH 窗格中心（Ctrl/Cmd+P 搜索页）的窗格详细描述合同：描述解析链（本地 i18n key → descriptor presentation）、描述参与搜索匹配、行内描述展示、宿主摘要行描述、窗格详情条交互（键盘、焦点、aria、Escape 嵌套）与安全边界。归档自 change dsh-pane-center-descriptions-v1（2026-08-28）。
## Requirements

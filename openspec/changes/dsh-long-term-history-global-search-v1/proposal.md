...
DSH 已经具备 append-only 会话日志、可选 SQLite 持久化、SQLite FTS 查询与 Web 侧栏内容搜索，但这些能力目前没有形成一个可发现、默认可长期保留、可搜索标题与标签、可跳转到命中位置、并由 Web 与 CLI 共用的产品合同。用户需要像 Codex 桌面端一样从统一入口找回过去对话，也需要像 Claude/Claude Code 一样搜索并恢复历史会话；如果继续让各入口各自拼接列表和索引，会产生重复状态、搜索范围不一致与无法迁移的前端私有实现。
...
- `dsh-history-client-plugins`: 定义 Web 命令面板/全局对话框与 CLI 的共享结果、receipt 和故障降级。
...
- canonical owner：DSH 上游（deepseek-ai/deepseek-harness；monorepo 自 2026-08-20 起不再维护 `client/deepseek-harness` fork，seam 经本仓 `upstream-prs/` 通道）的 Session、Persistence、Session Query、Apiproxy、Client Runtime、Web UI 与 CLI packages。
...
- 现有稳定面：`session.search` RPC、`SessionPersistence`、`ctx.sessionQuery`、`session/title`、Workspace archive set、Web sidebar search。
- 新增稳定面：`history.*` RPC/CLI、标签事件/服务、查询结果与 cursor、search document provider registry、Web command ids、派生索引 schema/config。

...
- [ ] 3.7 P0 阶段 B 验收：真实 owner 能力可用时执行分页／命中打开；不可用时只验适配合同，保留对应功能任务未完成。依赖：3.1–3.6；验收：证据明确区分实际查询与 mock。 Recheck 2026-09-09: `[external-gate skipped]` 维持——live history owner 仍缺（alpha 0.1.5-alpha.1 dsh-session/dsh-agent tarball 全量 grep historySearch/searchHistory/long-term-history 0 命中，next 仍 0.1.2-rc.1）；只验适配合同的证据边界不变，功能任务保持未勾。（dsh-session 0.1.5-rc.2 无 longTermHistory/historySearch API） Recheck 2026-09-11: `[external-gate skipped]` 维持——npm `@deepseek-ai/*` next 前移至 0.1.5-rc.2（temp/alpha-grep-0911/ 六包 tarball 实测）：preview/rendition/inspect、watchDirectory/FileWatch、ArtifactRef、standingFacts、mcp-inventory、workbench/panel host slot、ctx.media/attachments 扩展、client 侧 terminals registry（dsh-web/dsh-session/dsh-agent）全部 0 命中；host-apiproxy next 仍 0.1.1-rc.2。

...
| 6 | 会话侧栏 SessionSidebar / GlobalSearch | session | `packages/client/ui-desktop-workbench` + `packages/host/dsh-session-manager` | **probe 降级（seam 未接线）** | `ui-desktop-workbench/src/client/session-sidebar.tsx:78` / `global-search.tsx:43` 依赖 `host.listSessions()`；生产默认 host 为 placeholder：`dsh-session-manager/src/index.ts:146-153`（`not_implemented`/`return []`）、bundle `dsh-desktop-workbench/src/index.ts:33` 与 `src/client/composed-workbench.tsx:76` 默认 placeholder、`desktop-workbench-shell.tsx:92` fallbackHost 空表。注：composed overlay 为 deprecated，生产 apply 只注册 ConversationManagerPane，此侧栏当前不在主路径 | `SessionManagerHostV1` 生产 adapter（官方 `sessionPersistence`/`workspaceRegistry` → listSessions/archive/restore/fork） |
...
- 真数据：#3、#4、#5、#9 → **4 / 10 = 40%**
- probe 降级：#1、#2、#6、#7、#8 → 5
- 静态演示数据：#10 → 1
- **基线 = 40%**（R11 副指标目标 ≥80%；差距 4 个面板）
...
| #6 会话侧栏 | `SessionManagerHostV1` 生产 adapter（官方 `sessionPersistence`/`workspaceRegistry`/`storageDomain`） | 官方已有 seam 未接线 | 3.1 评估接线；未接线前保留空表+not_implemented 诚实态 |

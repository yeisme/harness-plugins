...
- [x] 3.5 用仓库 CLI/application service 生成 manifest/profile/compatibility metadata。Evidence (2026-08-29): `scripts/generate-bundle-metadata.mjs` 从代码源与 Bridge V2 fixture manifest 生成 `package.json`、`cordis.patch.yml`、`dsh.compatibility.json`；`pnpm --dir packages/bundle/dsh-ai-drama-director run check:metadata` 与 bundle declaration tests 4/4 通过。
- [x] 3.6 实现 install/uninstall/reinstall 幂等与完整 dispose。Evidence (2026-08-29): built ModuleLoader face smoke 验证 duplicate install no-op、uninstall 清除 6 views/10 commands/client face、reinstall 精确恢复一组注册；profile lifecycle/pack conformance PASS，证据位于 `temp/integration-test-runs/2026-08-29T11-13-53-203Z-1311520-drama-director-profile/summary.json`。
...
- [x] 5.3 pnpm run check:bundles。Evidence (2026-08-29): workspace root `pnpm -w run check:bundles` reports 24/24 bundles PASS。

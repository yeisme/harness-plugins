...
- [ ] 4.4 建立组件截图基线（browser gate：360/560/960px、dark、reduced-motion 截图基线属 V2 canary 浏览器验证波次，本仓无浏览器门依赖，如实未勾）：360/560/960px、dark、reduced-motion、text/source/image/table/editor、缺 capability、More 展开和 Bottom Sheet。
  - 2026-09-03 [external-gate skipped]：真实浏览器截图基线在本环境跳过（V2 canary 浏览器验证波次；plugin-host-protocol 完成门明文不作为插件完成条件）。归档前复跑等价 jsdom/断点面全绿：ui-interaction-space 92/92（view/layer/preferences/registry/contracts 含 560px 断点、Bottom Sheet、aria/focus、reduced-motion media 断言）、selection-annotation 46/46。维持未勾，截图基线留待 canary 波次真机补齐。
...
- [ ] 7.3 增加 Playwright（browser gate：journeys/screenshots 属 canary 浏览器验证波次，plugin-host-protocol 明文不作为插件完成条件；jsdom 层等价覆盖已交付，如实未勾） journeys/screenshots：360/560/960px、dark/reduced-motion、text/source/image/table/editor、coarse pointer、敏感/opt-out、More disabled reason、HMR/dispose。
  - 2026-09-03 [external-gate skipped]：Playwright journeys/screenshots 在本环境跳过（canary 浏览器验证波次，同上完成门口径）。jsdom 等价闭环复跑：ui-interaction-space 92/92（含 `tests/integration/flow.spec.tsx` 选择→stable Actions→typed intent→owner receipt、V1 adapter fallback、Esc/outside/Pin）、selection-annotation 46/46、`check:bundles` 27/27、strict validate PASS；HMR/dispose 归 runtime refcount + dispose-hmr-conformance 零 findings（2026-09-03T122937338Z 报告）。维持未勾，journeys 留待 canary 波次真机补齐。

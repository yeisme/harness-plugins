...
- [x] 2.1 实现Host来源适配与授权正文读取；区分同名不同安装包、项目/全局scope；正文不入目录snapshot或日志。 | evidence: readSkill 主体为 search-center 先决增量（owner list/getDocument 授权读取、FS 权限映射 denied/permission_denied、正文/目录/日志零正文投影）；本轮修复同名跨 source 误伤——matches 与读取后复检均按 name+source 过滤：同名异 source 安装互不串读（各自 resourceRef 不同）、同 source 双安装 fail-closed denied/source_not_readable；项目/全局由 source（project-*/user-*）区分、scope 保持 profile-only（1.2 冻结）。验证：tests/reference-reader.spec.ts 9/9（新增双 source 各读+同 source 拒绝+安装身份互异用例）、`pnpm --filter @yeisme/dsh-tool-hub-host run typecheck` exit 0、全包 32 passed（2 例 owner-gated 需 --host 集成证据入口，预存环境门非本项引入）。
- [ ] 2.2 实现引用解析与打开时授权检查；包内../、锚点、唯一行内文件名、编码越界、symlink与检查后替换；浏览器不传任意路径。（blocked 2026-09-16：引用解析需读取 SKILL.md 之外的包内文件；owner seam 现状=上游 `@deepseek-ai/dsh-skill-filesystem@0.1.5-rc.2` 无任何包文件读取面，`upstream-prs/skill-document-reader-v1` 补丁也仅新增 `getDocument(name)` 读单个 Skill 正文——包内相对文件读取 seam 缺失。实现前须先扩展该 upstream-prs 系列（或等上游发布），并保持缺 seam 时 `reader_unavailable` 诚实降级，不做浏览器侧路径兜底。）

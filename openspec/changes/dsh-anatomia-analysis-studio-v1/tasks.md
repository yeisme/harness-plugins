...
- [ ] 1.1 核对 Anatomia 公共接口与视频观察合同：时间坐标、来源/关键帧访问、范围分析、审阅、固定版本参考包逐项记录版本/digest、支持/缺失/未验证；与 agent/anatomia 双向链接。 | evidence: implementation-baseline.md：合同初查完成（owner HEAD f1e13ef）：微秒时间坐标/artifact授权访问/video-observation v1/video-analyses生命周期/review verify/revisions freeze+production-decomposition已逐项记录；范围分析两面确认不可用（inspection/comparison无默认route、understanding默认501）、delivery/long-form flag默认关。agent/anatomia侧无本change反向链接，owner侧配套须由anatomia-owner会话经5.1落地，故保持未勾。
- [x] 1.2 冻结最小分析台 consumer 合同与 UI Contract；核对脚手架复用；时间坐标换算收敛到 adapter 层。 | evidence: implementation-baseline.md：最小consumer合同冻结（video-analyses读面+固定revision的production-decomposition+artifact access grant+cancel动作；understanding/inspection/comparison/EvidenceCase明确排除）；微秒换算收敛adapter层写入冻结；UI Contract在design.md冻结；脚手架复用核对CreatorActionComposer/artifact-workspace/SurfaceState/rich-media renderer与host gateway。本切片无代码，openspec validate --strict 通过。

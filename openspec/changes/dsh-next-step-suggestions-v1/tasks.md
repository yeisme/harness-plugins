...
- [ ] 6.1 [Owner: Harness Plugins + DSH Host；Scope: `packages/host/next-step-suggestions/`；Dependencies: 4.2] 实现 host 侧 `ctx.nextStepSuggestions` registry 与 `next-step-suggestions` projection，支持跨 profile/跨进程建议贡献。Acceptance: host registry 可被多个插件注册并安全投影；Validation: host contract tests。（retain-next: V1 使用 client-local source registry）
- [ ] 6.2 [Owner: Harness Plugins；Scope: `src/client/`；Dependencies: 3.2] 在助手消息尾部（`conversation.chat.turnTail`）增加建议入口。Acceptance: 历史消息也可点击建议填入 composer；Validation: component/e2e。（retain-next: 需要上游 turnTail seam 或额外 slot）
- [ ] 6.3 [Owner: Harness Plugins；Scope: `src/client/suggestion-composer.ts`；Dependencies: 3.2] 增加“替换/追加”用户偏好。Acceptance: 设置可切换；Validation: component tests。（retain-next: 当前默认追加）

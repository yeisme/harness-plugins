...
- [ ] 4.2 接入聊天/ToolView/Pane 官方 slot，注册 `RichMediaCard` 与媒体节点；验收：使用最窄官方 seam，无 DOM patch。（progress 2026-08-22：PreviewResourceV1 fork PR #7 已登记；本任务仍等官方 Workbench/Pane/ToolView 宿主 slot，不在本 program 解锁范围内。（blocked 2026-08-24: official Workbench/Pane/ToolView host slot. PreviewResourceV1 fork PR is not an official DSH seam.） [external-gate skipped]（复核 2026-09-03：官方 Workbench/Pane/ToolView host slot、各 owner seam、ArtifactRef/领域 Pane seam、媒体附件 host seam（ctx.attachments/ctx.media）在已发布 `@deepseek-ai` 包至 0.1.2-rc.1（next）均未提供，workbench/panels host-slot 包在 registry 不存在。） Recheck 2026-09-04: `[external-gate skipped]` 维持——官方 Workbench/Pane/ToolView host slot 仍未发布（`@deepseek-ai` 至 0.1.2-rc.1 无对应面）。
...
- [ ] 4.5 将工作台文件/终端/Git/浏览器 Tab 接入各自官方 owner seam；验收：每个 Tab 只投影 owner 状态，不复制 canonical state。（blocked: 需要各自官方 owner seam） [external-gate skipped]（复核 2026-09-03：官方 Workbench/Pane/ToolView host slot、各 owner seam、ArtifactRef/领域 Pane seam、媒体附件 host seam（ctx.attachments/ctx.media）在已发布 `@deepseek-ai` 包至 0.1.2-rc.1（next）均未提供，workbench/panels host-slot 包在 registry 不存在。） Recheck 2026-09-04: `[external-gate skipped]` 维持——各 Tab 官方 owner seam 均未发布。
- [ ] 4.6 与 Eikona/Sonora/Anatomia 等领域 Pane 集成；验收：跨 Pane 只走 typed artifact intent。（blocked: 官方 ArtifactRef/领域 Pane seam 未就绪） [external-gate skipped]（复核 2026-09-03：官方 Workbench/Pane/ToolView host slot、各 owner seam、ArtifactRef/领域 Pane seam、媒体附件 host seam（ctx.attachments/ctx.media）在已发布 `@deepseek-ai` 包至 0.1.2-rc.1（next）均未提供，workbench/panels host-slot 包在 registry 不存在。） Recheck 2026-09-04: `[external-gate skipped]` 维持——官方 ArtifactRef/领域 Pane seam 未就绪（上游全树 0 命中）。
...
- [ ] 6.1 [Owner: Harness Plugins；Scope: `src/host/plugin.ts`；Dependencies: 2.2] 接入真实媒体附件 seam（`ctx.attachments` 扩展或 `ctx.media` 新增）。Acceptance: Host 返回真实 MediaRef；Validation: 本地 DSH profile 验证。（blocked: 需要真实 DSH 媒体附件 host seam，本环境无法执行本地 DSH profile） [external-gate skipped]（复核 2026-09-03：官方 Workbench/Pane/ToolView host slot、各 owner seam、ArtifactRef/领域 Pane seam、媒体附件 host seam（ctx.attachments/ctx.media）在已发布 `@deepseek-ai` 包至 0.1.2-rc.1（next）均未提供，workbench/panels host-slot 包在 registry 不存在。） Recheck 2026-09-04: `[external-gate skipped]` 维持——官方媒体附件 host seam（ctx.attachments 扩展/ctx.media）未提供（`dsh-attachment` 至 0.1.2-rc.1 仍 host 内部 image store）。 运行时复核 2026-09-04（dsh:dev 预览）：官方 web host 0.1.1-rc.2 + 本仓 25 bundle 全量 boot（67 entries，零装载失败）后 playwright 全页面探测 seam 关键词（fs.tree/terminal.attach/preview.rendition/media/attachments.list/ArtifactRef/standingFactsFor）全部 0 命中——静态结论经运行时证实。证据 temp/integration-test-runs/dsh-dev-runtime-probe-final-20260904T052115Z-2865128/。

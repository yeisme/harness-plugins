...
- [ ] 2.2 Owner: Harness Plugins；Lane: profile-composition；Dependencies: 1.2；Scope: bundle patches、package conformance、profile fixtures；Exclusions: 不发布或修改 production state；Acceptance: unified Ordo、independent composition、Pane Workbench rows 可被 clean profile 解析且无 duplicate/unresolved row；Validation: packed local profile test。
...
- 2.2、3.1、3.2 保持未完成：Harness 侧 profile packed composition 尚未能对齐已安装 DSH peer 的 standing/source read API；Pane bundle/profile browser evidence 也未完成。DSH E2E evidence 位于 `client/deepseek-harness/temp/integration-test-runs/20260816T062748Z-dsh-composition-e2e/`，不能替代 Harness profile evidence。

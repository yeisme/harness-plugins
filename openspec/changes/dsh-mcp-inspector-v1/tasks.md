...
- [ ] 3.1 L2 seam `upstream-prs/mcp-inventory/`：patch + apply.sh + 推 fork pr 分支（用户已确认留待专门会话；设计见 design.md）。 Recheck 2026-09-04: `[external-gate skipped]` 维持——用户已确认留待专门会话；`upstream-prs/mcp-inventory/` 仍未创建（上游全树 mcp-inventory 0 命中）。
